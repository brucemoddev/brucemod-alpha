...
/*********************************************************************
 **  Function: setup_brucemod
 **  Create necessary folders for BruceMod
 *********************************************************************/
void setup_brucemod() {
    if (SD.cardSize() > 0) {
        if (!SD.exists("/BruceMod")) SD.mkdir("/BruceMod");
        if (!SD.exists("/BruceMod/IR")) SD.mkdir("/BruceMod/IR");
        if (!SD.exists("/BruceModRFID")) SD.mkdir("/BruceModRFID");
        if (!SD.exists("/BruceModPCAP")) SD.mkdir("/BruceModPCAP");
        if (!SD.exists("/BruceModJS")) SD.mkdir("/BruceModJS");
        if (!SD.exists("/BruceModWebUI")) SD.mkdir("/BruceModWebUI");
        if (!SD.exists("/BruceModScripts")) SD.mkdir("/BruceModScripts");
        if (!SD.exists("/BruceModMIC")) SD.mkdir("/BruceModMIC");
        if (!SD.exists("/BruceModGPS")) SD.mkdir("/BruceModGPS");
        if (!SD.exists("/BruceModWardriving")) SD.mkdir("/BruceModWardriving");
        if (!SD.exists("/BruceModIR")) SD.mkdir("/BruceModIR");
        if (!SD.exists("/BruceModRF")) SD.mkdir("/BruceModRF");
        if (!SD.exists("/BruceModEvilCreds")) SD.mkdir("/BruceModEvilCreds");
...
void boot_screen() {
    ...
    tft.drawCentreString("BruceMod", tftWidth / 2, 10, 1);
    tft.setTextSize(FP);
...
void setup() {
    ...
    if (!bruceConfig.batteryWarningShown) {
        tft.fillScreen(TFT_BLACK);
        tft.setTextColor(TFT_RED, TFT_BLACK);
        tft.drawCentreString("!!! WARNING !!!", tft.width() / 2, 20, 2);
        tft.setTextColor(TFT_WHITE, TFT_BLACK);
        tft.setTextSize(FP);
        tft.drawCentreString("Voltage is raw hardware data.", tft.width() / 2, 50, 1);
        tft.drawCentreString("Expect fluctuations under load.", tft.width() / 2, 65, 1);
        tft.drawCentreString("Press ANY KEY to continue.", tft.width() / 2, 100, 1);
        while (!check(AnyKeyPress)) { vTaskDelay(10 / portTICK_PERIOD_MS); }
        bruceConfig.setBatteryWarningShown(true);
        tft.fillScreen(TFT_BLACK);
    }
