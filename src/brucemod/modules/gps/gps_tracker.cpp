...
void GPSTracker::add_initial_file_data(File file) {
    ...
    file.println("  creator=\"BruceMod Firmware\"");
    file.println("  <metadata>");
    file.println("    <name>BruceMod GPS Tracker</name>");
    file.println("    <desc>GPS Tracker using BruceMod Firmware</desc>");
    file.println("    <link href=\"https://bruce.computer\">");
    file.println("      <text>BruceMod Website</text>");
    file.println("    </link>");
    file.println("  </metadata>");
    file.println("  <trk>");
    file.println("    <name>BruceMod route</name>");
    file.println("    <desc>GPS route captured by BruceMod firmware</desc>");
    file.println("    <trkseg>");
}
...
void GPSTracker::add_final_file_data() {
    ...
    if (filename == "" || !(*fs).exists("/BruceModGPS/" + filename)) return;

    File file = (*fs).open("/BruceModGPS/" + filename, FILE_APPEND);
...
void GPSTracker::add_coord() {
    ...
    if (!(*fs).exists("/BruceModGPS")) (*fs).mkdir("/BruceModGPS");

    bool is_new_file = false;
    if (!(*fs).exists("/BruceModGPS/" + filename)) is_new_file = true;
    File file = (*fs).open("/BruceModGPS/" + filename, is_new_file ? FILE_WRITE : FILE_APPEND);
