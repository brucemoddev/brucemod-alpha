...
bool Chameleon::readFileLF() {
    ...
    if (!(*fs).exists("/BruceModRFID")) (*fs).mkdir("/BruceModRFID");
    filepath = loopSD(*fs, true, "RFIDLF", "/BruceModRFID");
...
bool Chameleon::writeFileLF(String filename) {
    ...
    if (!(*fs).exists("/BruceModRFID")) (*fs).mkdir("/BruceModRFID");
    if ((*fs).exists("/BruceModRFID/" + filename + ".rfidlf")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists("/BruceModRFID/" + filename + String(i) + ".rfidlf")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/BruceModRFID/" + filename + ".rfidlf", FILE_WRITE);

    if (!file) { return false; }

    file.println("Filetype: BruceMod RFID 125kHz File");
...
bool Chameleon::readFileHF() {
    ...
    if (!getFsStorage(fs)) return false;
    if (!(*fs).exists("/BruceModRFID")) (*fs).mkdir("/BruceModRFID");
    filepath = loopSD(*fs, true, "RFID|NFC", "/BruceModRFID");
    file = fs->open(filepath, FILE_READ);
...
bool Chameleon::writeFileHF(String filename) {
    ...
    if (!(*fs).exists("/BruceModRFID")) (*fs).mkdir("/BruceModRFID");
    if ((*fs).exists("/BruceModRFID/" + filename + ".rfid")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists("/BruceModRFID/" + filename + String(i) + ".rfid")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/BruceModRFID/" + filename + ".rfid", FILE_WRITE);

    if (!file) { return false; }

    file.println("Filetype: BruceMod RFID File");
    file.println("Version 1");
...
void Chameleon::saveScanResult() {
    ...
    if (!(*fs).exists("/BruceModRFID")) (*fs).mkdir("/BruceModRFID");
    if (!(*fs).exists("/BruceModRFID/Scans")) (*fs).mkdir("/BruceModRFID/Scans");
    if ((*fs).exists("/BruceModRFID/Scans/" + filename + ".rfidscan")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists("/BruceModRFID/Scans/" + filename + String(i) + ".rfidscan")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/BruceModRFID/Scans/" + filename + ".rfidscan", FILE_WRITE);

    if (!file) { return; }

    file.println("Filetype: BruceMod RFID Scan Result");
