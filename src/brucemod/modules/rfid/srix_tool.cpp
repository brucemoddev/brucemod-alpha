...
void SRIXTool::save_file() {
    ...
    if (!(*fs).exists("/BruceModRFID/SRIX")) (*fs).mkdir("/BruceModRFID/SRIX");

    // Check if the file already exists and add number
    String filepath = "/BruceModRFID/SRIX/" + filename;
    ...
    // Write header
    file.println("Filetype: BruceMod SRIX Dump");
...
void SRIXTool::load_file() {
    ...
    if (!(*fs).exists("/BruceModRFID/SRIX")) {
    ...
    File dir = (*fs).open("/BruceModRFID/SRIX");
    ...
    for (const String &fname : fileList) {
        options.emplace_back(fname, [this, fname, fs]() { load_file_data(fs, "/BruceModRFID/SRIX/" + fname); });
    }
...
void SRIXTool::load_file_data(FS *fs, String filepath) {
    ...
    int lastSlash = filename.lastIndexOf('/');
    if (lastSlash >= 0) {
        filename = filename.substring(lastSlash + 1); // Remove "/BruceModRFID/SRIX/"
    }
