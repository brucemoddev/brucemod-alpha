...
void EvilPortal::saveToCSV(const String &csvLine, bool isAPname) {
    ...
    if (!fs->exists("/BruceModEvilCreds")) fs->mkdir("/BruceModEvilCreds");

    File file;

    if (!isAPname) {
        file = fs->open("/BruceModEvilCreds/" + outputFile, FILE_APPEND);
    } else {
        file = fs->open("/BruceModEvilCreds/" + apName + "_creds.csv", FILE_APPEND);
    }
