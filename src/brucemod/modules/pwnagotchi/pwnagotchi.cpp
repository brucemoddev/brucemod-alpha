...
void brucemodgotchi_setup() {
    initPwngrid();
    initUi();
    state = STATE_INIT;
    Serial.println("BruceModgotchi Initialized");
}

void brucemodgotchi_update() {
...
void brucemodgotchi_start() {
    ...
    if (setupSdCard()) {
        ...
        if (!SD.exists("/BruceModPCAP/handshakes")) SD.mkdir("/BruceModPCAP/handshakes");
        handshakeFs = &SD;
    } else {
        if (!LittleFS.exists("/BruceModPCAP")) LittleFS.mkdir("/BruceModPCAP");
        if (!LittleFS.exists("/BruceModPCAP/handshakes")) LittleFS.mkdir("/BruceModPCAP/handshakes");
        isLittleFS = true;
    ...
    brucemodgotchi_setup(); // Starts the thing
    // Draw footer & header
    drawTopCanvas();
    drawBottomCanvas();
