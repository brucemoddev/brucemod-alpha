...
// Internal helper to avoid code duplication and constant init/deinit
static void runLiveAppleSpam(int payloadIndex, bool cycleAll) {
    apple_spam_running = true;
    BLEDevice::init("");
    
    NimBLEAdvertising* pAdv = BLEDevice::getAdvertising();
    int apple_index = payloadIndex;
    ...
    while (apple_spam_running) {
        ...
        // Randomize MAC for every burst to be more effective
        uint8_t macAddr[6];
        ...
        advData.addData(fullPayload, apple_payloads[apple_index].length + 2);
        
        pAdv->setAdvertisementData(advData);
        pAdv->start();
        
        displayTextLine(String(apple_payloads[apple_index].name) + " Spaming...");
...
void quickAppleSpam(int payloadIndex) {
    ...
    advData.addData(fullPayload, apple_payloads[payloadIndex].length + 2);
...
void startAppleSpamAll() {
    ...
    padprintln("Cycling all payloads...");
    padprintln("Press ESC to stop");
    runLiveAppleSpam(0, true);
    returnToMenu = true;
}
...
void startAppleSpam(int payloadIndex) {
    ...
    padprintln("Targeted spamming...");
    padprintln("Press ESC to stop");
    runLiveAppleSpam(payloadIndex, false);
    returnToMenu = true;
}
