...
bool rf_raw_save(RawRecording recorded) {
    ...
    if (!fs->exists("/BruceModRF")) {
        if (!fs->mkdir("/BruceModRF")) {
            displayError("Error creating directory", true);
            return false;
        }
    }

    do { snprintf(filename, sizeof(filename), "/BruceModRF/raw_%d.sub", index++); } while (fs->exists(filename));
    ...
    file.write((const uint8_t *)"Filetype: BruceMod SubGhz File\n", 28);
