...
void ReverseShell() {
    ...
    if (!WiFi.softAP("BruceModShell", "", 1)) {
        tft.println("Failed to start AP");
        return;
    }

    tft.println("Wi-Fi AP Started: BruceModShell");
    ...
    webServer.on("/", [&webServer]() {
        String html = R"rawliteral(
            ...
            <html>
            <head>
                <title>BruceModShell Web Interface</title>
                <style>
    ...
    while (true) {
        ...
        if (!shellConnected) {
            ...
            if (tcpClient) {
                ...
                tcpClient.println("~Welcome to BruceModShell.");
