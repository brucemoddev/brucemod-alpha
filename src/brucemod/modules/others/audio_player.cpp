...
void drawVectorIcon(int x, int y, int size, IconType icon, uint16_t color) {
    ...
    switch (icon) {
        ...
        case ICON_LOOP_OFF: {
            ...
            tft.drawCircle(cx, cy, r, bruceConfig.secColor);
            // Arrow tip to indicate loop direction
            tft.fillTriangle(cx + r - 3, cy - 3, cx + r + 3, cy - 3, cx + r, cy + 3, bruceConfig.secColor);
...
void drawButton(int x, int y, int size, IconType icon, bool selected, bool active) {
    ...
    uint16_t iconColor = active ? bruceConfig.priColor : TFT_DARKGREY;
    uint16_t borderColor = selected ? bruceConfig.priColor : bruceConfig.secColor;

    // Highlighted button has colored background
    if (selected) {
        bgColor = bruceConfig.priColor;
        iconColor = bruceConfig.bgColor;
        tft.fillRoundRect(x, y, size, size, 6, bgColor);
        tft.drawRoundRect(x, y, size, size, 6, bruceConfig.secColor);
    } else {
        tft.fillRoundRect(x, y, size, size, 6, bgColor);
...
bool showVolumeControl(uint8_t &currentVolume) {
    ...
    auto drawVolumeBar = [&]() {
        // Dark overlay
        tft.fillRect(BAR_X - 2, BAR_Y - 2, BAR_WIDTH + 4, BAR_HEIGHT + 4, bruceConfig.bgColor);

        // Container
        tft.drawRoundRect(BAR_X, BAR_Y, BAR_WIDTH, BAR_HEIGHT, 8, bruceConfig.priColor);

        // Speaker icon at top
        drawVectorIcon(BAR_X + (BAR_WIDTH - 20) / 2, BAR_Y + 10, 20, ICON_VOL, bruceConfig.priColor);
        ...
        uint16_t volColor = (tempVolume > 80) ? TFT_RED : (tempVolume < 20 ? bruceConfig.secColor : bruceConfig.priColor);
...
void musicPlayerUI(FS *fs, const String &filepath) {
    ...
    auto drawTrackInfo = [&]() {
        ...
        tft.drawRoundRect(ui.MARGIN_X, y + 2, boxSize, iconSize, 4, bruceConfig.secColor);
