...
bool ARPSpoofer::arpPCAPfile() {
    ...
    while (fs->exists(String("/BruceModPCAP/ARP_session_" + String(nf++) + ".pcap").c_str())) yield();
    pcapFile = fs->open(String("/BruceModPCAP/ARP_session_" + String(nf) + ".pcap").c_str(), FILE_WRITE);
...
void ARPSpoofer::setup(const Host &host, IPAddress gateway) {
    ...
    if (mitm) {
        ...
        // padprintln("/BruceModPCAP/ARP_session_" + String(nf) + ".pcap");
