...
bool IrRead::write_file(String filename, FS *fs) {
    ...
    if (!(*fs).exists("/BruceModIR")) (*fs).mkdir("/BruceModIR");

    while ((*fs).exists("/BruceModIR/" + filename + ".ir")) {
        int ch = 1;
        int i = 1;
        ...
        switch (ch) {
            case 1:
                filename += "_";
                while ((*fs).exists("/BruceModIR/" + filename + String(i) + ".ir")) i++;
                filename += String(i);
                break;
            case 2: (*fs).remove("/BruceModIR/" + filename + ".ir"); break;
            case 3:
    ...
    if ((*fs).exists("/BruceModIR/" + filename + ".ir")) {
        int i = 1;
        filename += "_";
        while((*fs).exists("/BruceModIR/" + filename + String(i) + ".ir")) i++;
        filename += String(i);
    ...
    */
    ...
    File file = (*fs).open("/BruceModIR/" + filename + ".ir", FILE_WRITE);

    if (!file) { return false; }

    file.println("Filetype: BruceMod IR File");
