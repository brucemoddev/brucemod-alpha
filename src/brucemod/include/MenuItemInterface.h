...
public:
    ...
    void draw(float scale = 1, bool skipClear = false) {
        if (rotation != bruceConfigPins.rotation) resetCoordinates();
        if (!checkTheme()) {
            if (!skipClear) tft.fillRect(0, 27, tftWidth, tftHeight - 27, bruceConfig.bgColor);
            drawIcon(scale);
            if (!skipClear) {
                drawArrows(scale);
                drawTitle(scale);
            ...
            if (bruceConfig.theme.label && !skipClear)
                drawTitle(scale); // If using .GIF, labels are draw after complete, which takes some time
            drawIconImg();
            if (bruceConfig.theme.label && !skipClear) drawTitle(scale); // Makes sure to draw over the image
        }
        if (!skipClear) drawStatusBar();
    }

    void draw(float scale, int x, int y, bool skipClear = true) {
        // Custom draw usually for small icons in DSi/PS4 styles
        // Temporarily override coordinates
        int oldCX = iconCenterX;
        int oldCY = iconCenterY;
        int oldX = iconAreaX;
        int oldY = iconAreaY;
        
        iconCenterX = x;
        iconCenterY = y;
        iconAreaX = x - (iconAreaW * scale) / 2;
        iconAreaY = y - (iconAreaH * scale) / 2;

        _isCustomDraw = skipClear;
        if (!checkTheme()) {
            drawIcon(scale);
        } else {
            drawIcon(scale); 
