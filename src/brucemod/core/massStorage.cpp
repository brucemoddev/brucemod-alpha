...
uint64_t MassStorage::totalBytes = 0;
uint32_t MassStorage::lastSpeedUpdate = 0;
...
void MassStorage::loop() {
    ...
    while (!check(EscPress) && !shouldStop) {
        uint32_t now = millis();
        if (now - lastSpeedUpdate >= 20000) { // 20 seconds
            float speed = (float)totalBytes / 1024.0f / ((now - lastSpeedUpdate) / 1000.0f);
            displaySpeed(speed);
            totalBytes = 0;
            lastSpeedUpdate = now;
        }
...
void MassStorage::displaySpeed(float speed) {
    // We don't want to clear the whole screen, just update the speed line
    // The icon is drawn in the middle (tftHeight / 2)
    // displayMessage adds a line. Let's draw it at the bottom.
    tft.fillRect(5, tftHeight - 20, tftWidth - 10, 15, bruceConfig.bgColor);
    tft.setTextColor(bruceConfig.priColor, bruceConfig.bgColor);
    tft.setCursor(10, tftHeight - 20);
    tft.printf("Avg Speed: %.2f kB/s", speed);
}
