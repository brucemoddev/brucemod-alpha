...
void padprintln(const String &s, int16_t padx) {
    ...
    if (_maxCharsInLine <= 0) return;

    // automatically split into multiple lines
    while (start < s.length()) {
        size_t len = std::min((size_t)_maxCharsInLine, s.length() - start);
        tft.setCursor(padx * BORDER_PAD_X, tft.getCursorY());
...
void padprintln(const char str[], int16_t padx) {
    ...
    // automatically split into multiple lines
    while (start < s_len) {
        size_t len = std::min((size_t)_maxCharsInLine, s_len - start);
        tft.setCursor(padx * BORDER_PAD_X, tft.getCursorY());
        for (size_t i = 0; i < len; ++i) {
            tft.print(str[start + i]);
        }
        tft.println();
        start += len;
    }
...
void showHelpPopup(const char* title) {
    const char* text = getHelpText(title);
    ScrollableTextArea area = ScrollableTextArea("HELP: " + String(title));
    area.fromString(String(text));
    area.show();
}

int loopOptions(std::vector<Option> &options, uint8_t menuType, const char *subText, int index, bool interpreter) {
    Opt_Coord coord;
    bool redraw = true;
    bool exit = false;
    ...
    showHelpHint = true; // Ensure help hint is shown when entering a menu
    drawMainBorder();
    while (1) {
        ...
        if (menuType == MENU_TYPE_SUBMENU && tft.textWidth(options[index].label) > tftWidth - 20) {
            if (millis() - lastScrollRedraw > 150) {
                lastScrollRedraw = millis();
                redraw = true;
            }
        }
        ...
        if (redraw) {
            ...
            if (options[index].hover && bruceConfig.menuStyle == 0)
                renderedByLambda = options[index].hover(options[index].hoverPointer, true);

            if (!renderedByLambda) {
                if (menuType == MENU_TYPE_SUBMENU || menuType == MENU_TYPE_MAIN) {
                    int menuTypeStyle = (menuType == MENU_TYPE_MAIN) ? bruceConfig.menuStyle : bruceConfig.subMenuStyle;
                    switch(menuTypeStyle) {
                        case 1: drawWiiMenu(index, options, subText); break;
                        case 2: drawDsiMenu(index, options, subText); break;
                        case 3: drawPs4Menu(index, options, subText); break;
                        case 4: drawVerticalMenu(index, options, subText); break;
                        case 5: drawC64Menu(index, options, subText); break;
                        default:
        ...
        // Help popup [1] and Shortcuts
        keyStroke _key = _getKeyPress();
        if (_key.pressed && !_key.word.empty()) {
            if (_key.word[0] == '1') {
                showHelpPopup(subText);
                redraw = true;
            } else {
#ifdef HAS_KEYBOARD
                processShortcuts(_key);
#endif
            }
        }
...
Opt_Coord drawOptions(
    ...
    // Full redraw if first time, or if the sliding window of items changed
    if (firstRender || init != lastInit) {
        tft.fillRoundRect(
            tftWidth * 0.10, optionsTopY, tftWidth * 0.8, (FM * 8 + 4) * menuSize + 10, 5, bgcolor
        );
        tft.drawRoundRect(
            tftWidth * 0.10, optionsTopY, tftWidth * 0.8, (FM * 8 + 4) * menuSize + 10, 5, fgcolor
        );
        
        tft.setTextSize(FM);
        int cont = 0;
        for (int i = init; i < options.size() && cont < MAX_MENU_SIZE; i++, cont++) {
            tft.setCursor(tftWidth * 0.10 + 5, optionsTopY + 5 + cont * (FM * 8 + 4) + 4);
            ...
            int maxChars = (tftWidth * 0.8 - 10) / (LW * FM) - 1;
            tft.println(text.substring(0, maxChars));
            
            if (i == index) {
                coord.x = tftWidth * 0.10 + 5 + FM * LW;
                coord.y = tft.getCursorY() - (FM * 8 + 4) + 4; // approximate
                coord.size = maxChars;
        ...
        // Partial redraw: only old selection and new selection
        tft.setTextSize(FM);
        int maxChars = (tftWidth * 0.8 - 10) / (LW * FM) - 1;

        // Erase old cursor
        if (lastIndex >= init && lastIndex < init + MAX_MENU_SIZE) {
            int oldCont = lastIndex - init;
            tft.setCursor(tftWidth * 0.10 + 5, optionsTopY + 5 + oldCont * (FM * 8 + 4) + 4);
            tft.setTextColor(options[lastIndex].selected ? selcolor : fgcolor, bgcolor);
            String text = " " + options[lastIndex].label;
            tft.println(text.substring(0, maxChars));
        }

        // Draw new cursor
        int newCont = index - init;
        tft.setCursor(tftWidth * 0.10 + 5, optionsTopY + 5 + newCont * (FM * 8 + 4) + 4);
        tft.setTextColor(options[index].selected ? selcolor : fgcolor, bgcolor);
        String text = ">" + options[index].label;
        tft.println(text.substring(0, maxChars));
        
        coord.x = tftWidth * 0.10 + 5 + FM * LW;
        coord.y = tft.getCursorY() - (FM * 8 + 4) + 4;
        coord.size = maxChars;
        coord.fgcolor = fgcolor;
        coord.bgcolor = bgcolor;
    }
...
void drawFullLabelArea(String label) {
    static String lastLabel = "";
    if (label == lastLabel) return;
    lastLabel = label;

    // Area at the bottom to show full name of truncated items
    tft.fillRect(0, tftHeight - 12, tftWidth, 12, bruceConfig.bgColor);
    tft.drawFastHLine(0, tftHeight - 13, tftWidth, bruceConfig.secColor);
    tft.setTextColor(bruceConfig.priColor, bruceConfig.bgColor);
    tft.setTextSize(1);
    
    if (showHelpHint) {
        tft.setCursor(5, tftHeight - 10);
        tft.print("[1]:Help");
    }
    
    tft.drawCentreString(label, tftWidth / 2, tftHeight - 10, 1);
}
...
void drawSubmenu(int index, std::vector<Option> &options, const char *title) {
    ...
    int menuSize = options.size();
    uint16_t priColor = bruceConfig.priColor;
    ...
    bool fullRedraw = (lastIndex == -1) || (lastTitle != String(title));

    if (fullRedraw) {
        drawStatusBar(true);
        tft.setTextColor(priColor, bgColor);
        tft.setTextSize(FP);
        tft.fillRect(6, 30, tftWidth - 12, 8 * FP, bgColor);
        tft.drawString(title, 12, 30);
    }

    if (fullRedraw || index != lastIndex) {
        tft.setTextSize(FM);
#if defined(HAS_TOUCH)
        if (fullRedraw) tft.drawCentreString("/\\", tftWidth / 2, middle_up - (FM * LH + 6), 1);
#endif
        // Previous item
        const char *firstOption =
            index - 1 >= 0 ? options[index - 1].label.c_str() : options[menuSize - 1].label.c_str();
        tft.setTextColor(getColorVariation(priColor));
        tft.fillRect(6, middle_up, tftWidth - 12, 8 * FM, bgColor);
        ...
        tft.setTextColor(priColor);
        tft.fillRect(6, middle - FG * LH / 2 - 1, tftWidth - 12, FG * LH + 5, bgColor);
        
        String label = options[index].label;
        int labelWidth = tft.textWidth(label);

        if (labelWidth > tftWidth - 20) {
            int maxChars = (tftWidth - 20) / (selectedTextSize * LW);
            tft.drawString(label.substring(0, maxChars), 10, middle - selectedTextSize * LH / 2, SMOOTH_FONT);
        } else {
            tft.drawCentreString(label, tftWidth / 2, middle - selectedTextSize * LH / 2, SMOOTH_FONT);
        }

        tft.drawFastHLine(
            tftWidth / 2 - label.length() * selectedTextSize * LW / 2,
            middle + selectedTextSize * LH / 2 + 1,
            label.length() * selectedTextSize * LW,
            priColor
        );
        ...
        tft.setTextSize(FM);
        tft.setTextColor(getColorVariation(priColor));
        tft.fillRect(6, middle_down, tftWidth - 12, 8 * FM, bgColor);
        tft.drawCentreString(thirdOption, tftWidth / 2, middle_down, SMOOTH_FONT);

        // Scroll bar
        tft.fillRect(tftWidth - 5, 0, 5, tftHeight, bgColor);
        tft.fillRect(tftWidth - 5, index * tftHeight / menuSize, 5, tftHeight / menuSize, priColor);

        drawFullLabelArea(options[index].label);
    }
...
void drawC64Menu(int index, std::vector<Option> &options, const char *title) {
    uint16_t c64_bg = bruceConfig.bgColor; 
    uint16_t c64_fg = bruceConfig.priColor; 
    
    tft.fillScreen(c64_fg);
    tft.fillRect(10, 10, tftWidth - 20, tftHeight - 20, c64_bg);
    
    tft.setTextColor(c64_fg, c64_bg);
    tft.setTextSize(1);
    
    tft.drawCentreString("**** BRUCEMOD 64 BASIC V2 ****", tftWidth / 2, 15, 1);
    
    char memMsg[32];
    sprintf(memMsg, "%lu BASIC BYTES FREE", (unsigned long)ESP.getFreeHeap());
    tft.drawCentreString(memMsg, tftWidth / 2, 25, 1);
    
    int items_count = options.size();
    int start_index = (index / 10) * 10;
    
    for(int i = 0; i < 2; i++) {
        for(int j = 0; j < 5; j++) {
            int item_idx = start_index + i * 5 + j;
            if(item_idx >= items_count) continue;
            
            int x_off = 20 + i * (tftWidth / 2 - 10);
            int y_off = 40 + j * 12;
            
            bool selected = (item_idx == index);
            if(selected) {
                tft.fillRect(x_off - 2, y_off - 1, (tftWidth / 2 - 30), 10, c64_fg);
                tft.setTextColor(c64_bg, c64_fg);
            } else {
                tft.setTextColor(c64_fg, c64_bg);
            }
            
            String label = String(item_idx) + "." + options[item_idx].label;
            if(label.length() > 15) label = label.substring(0, 12) + "...";
            tft.drawString(label, x_off, y_off);
        }
    }
    
    tft.setTextColor(c64_fg, c64_bg);
    tft.drawString("READY.", 20, 105);
    
    if (showHelpHint) {
        tft.setTextColor(getColorVariation(c64_fg, 50, 1), c64_bg);
        tft.drawString("PRESS 1 FOR HELP", 65, 105);
    }

    // Show full name between READY and the end of box (around y=115)
    tft.setTextColor(getColorVariation(c64_fg, 30, 1), c64_bg);
    String fullLabel = "> " + options[index].label;
    if (fullLabel.length() > 30) fullLabel = fullLabel.substring(0, 27) + "...";
    tft.drawString(fullLabel, 20, 115);
}

void drawWiiMenu(int index, std::vector<Option> &options, const char *title) {
    drawMainBorder(true);
    int items_count = options.size();
    int start_index = (index / 6) * 6;
    ...
    for (int i = 0; i < 6; i++) {
        ...
        bool selected = (item_idx == index);
        if (selected) {
            tft.fillRoundRect(x_off, y_off, 70, 40, 5, bruceConfig.priColor);
            tft.setTextColor(bruceConfig.bgColor, bruceConfig.priColor);
        } else {
            tft.drawRoundRect(x_off, y_off, 70, 40, 5, bruceConfig.priColor);
            tft.setTextColor(bruceConfig.priColor, bruceConfig.bgColor);
        }
        
        String label = options[item_idx].label;
        if (label.length() > 10) label = label.substring(0, 8) + "..";
        tft.drawCentreString(label, x_off + 35, y_off + 15, 1);
    }
    drawFullLabelArea(options[index].label);
}

float currentIconScale = 1.0f;
int currentIconX = 0;
int currentIconY = 0;
bool currentIconCustom = false;

void drawDsiMenu(int index, std::vector<Option> &options, const char *title) {
    drawMainBorder(true);
    int items_count = options.size();
    
    for (int i = -2; i <= 2; i++) {
        int item_idx = (index + i + items_count) % items_count;
        int x_pos = tftWidth / 2 + i * 50;
        int y_pos = tftHeight / 2;
        
        if (i == 0) {
            tft.fillRoundRect(x_pos - 25, y_pos - 25, 50, 50, 5, bruceConfig.priColor);
            tft.setTextColor(bruceConfig.bgColor, bruceConfig.priColor);
            tft.drawCentreString(options[item_idx].label, x_pos, y_pos + 28, 1);
            
            // Draw Icon
            if (options[item_idx].hover) {
                currentIconScale = 0.4f;
                currentIconX = x_pos;
                currentIconY = y_pos;
                currentIconCustom = true;
                options[item_idx].hover(options[item_idx].hoverPointer, true);
                currentIconCustom = false;
            }
        } else {
            tft.drawRoundRect(x_pos - 20, y_pos - 20, 40, 40, 5, bruceConfig.secColor);
            if (options[item_idx].hover) {
                currentIconScale = 0.3f;
                currentIconX = x_pos;
                currentIconY = y_pos;
                currentIconCustom = true;
                options[item_idx].hover(options[item_idx].hoverPointer, true);
...
void drawPs4Menu(int index, std::vector<Option> &options, const char *title) {
    drawMainBorder(true);
    int items_count = options.size();
    
    tft.setTextColor(bruceConfig.priColor, bruceConfig.bgColor);
    tft.drawCentreString(options[index].label, tftWidth / 2, tftHeight - 20, 1);
    
    for (int i = -3; i <= 3; i++) {
        int item_idx = (index + i + items_count) % items_count;
        int x_pos = tftWidth / 2 + i * 40;
        int y_pos = tftHeight / 2 - 10;
        
        if (i == 0) {
            tft.fillRect(x_pos - 18, y_pos - 18, 36, 36, bruceConfig.priColor);
            if (options[item_idx].hover) {
                currentIconScale = 0.3f;
                currentIconX = x_pos;
                currentIconY = y_pos;
            ...
            tft.drawRect(x_pos - 15, y_pos - 15, 30, 30, bruceConfig.secColor);
            if (options[item_idx].hover) {
                currentIconScale = 0.25f;
                currentIconX = x_pos;
                currentIconY = y_pos;
                currentIconCustom = true;
                options[item_idx].hover(options[item_idx].hoverPointer, true);
...
void drawVerticalMenu(int index, std::vector<Option> &options, const char *title) {
    // This style is a bit tricky on Cardputer without actually rotating the screen
    // We'll just do a list with larger items
    drawMainBorder(true);
    int items_count = options.size();
    int start_index = max(0, index - 3);
    ...
    for (int i = 0; i < 7; i++) {
        ...
        int y_off = 30 + i * 15;
        if (item_idx == index) {
            tft.fillRect(5, y_off, tftWidth - 10, 14, bruceConfig.priColor);
            tft.setTextColor(bruceConfig.bgColor, bruceConfig.priColor);
        } else {
            tft.setTextColor(bruceConfig.priColor, bruceConfig.bgColor);
        }
        tft.drawString(options[item_idx].label, 10, y_off + 2);
    }
    drawFullLabelArea(options[index].label);
}
...
void drawStatusBar(bool force) {
    ...
    static String last_timeStr = "";
    static bool last_sd = false;
    static bool last_gps = false;
    static bool last_wifi = false;
    static bool last_webui = false;
    static bool last_ble = false;
    static bool last_wg = false;

#ifdef HAS_RGB_LED
    if (bruceConfig.statusLight) setLedColor(CRGB::Red);
#endif
    ...
    bool changed = (bat != last_bat) || (charging != last_charging) || 
                   (sdcardMounted != last_sd) || (gpsConnected != last_gps) ||
                   (WiFi.getMode() != last_wifi) || (isWebUIActive != last_webui) ||
                   (BLEConnected != last_ble) || (isConnectedWireguard != last_wg) ||
                   force;

    if (bruceConfig.debugMode) {
        if (current_heap != last_free_heap) changed = true;
    } else {
        if (String(timeStr) != last_timeStr) changed = true;
    }

    if (!changed) {
#ifdef HAS_RGB_LED
        if (bruceConfig.statusLight) {
            delay(10);
    ...
    last_timeStr = String(timeStr);
    last_sd = sdcardMounted;
    last_gps = gpsConnected;
    last_wifi = (WiFi.getMode() != 0);
    last_webui = isWebUIActive;
    last_ble = BLEConnected;
    last_wg = isConnectedWireguard;

    // Clear status bar area
    tft.fillRect(5, 5, tftWidth - 10, 20, bruceConfig.bgColor);

    int i = 0;
    uint8_t bat_margin = 125;
    if (bat > 0) {
    ...
    if (clock_set) {
        ...
        if (bruceConfig.debugMode) {
            char debugInfo[32];
            sprintf(debugInfo, "%luM %luK", (unsigned long)getCpuFrequencyMhz(), (unsigned long)current_heap/1024);
            tft.print(debugInfo);
        } else {
#if defined(HAS_RTC)
        ...
        tft.print("BruceMod " + String(BRUCE_VERSION));
    }
...
void drawBatteryStatus(uint8_t bat) {
    ...
    if (volt > 0.1) {
        tft.drawRightString(String(volt, 2) + "V", tftWidth - 85, 12, 1);
    }
