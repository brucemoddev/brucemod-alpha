...
bool checkUserWebAuth(AsyncWebServerRequest *request, bool onFailureReturnLoginPage = false) {
    if (request->hasHeader("Cookie")) {
        ...
        int idx = c.indexOf("BRUCEMODSESSION=");
        if (idx != -1) {
            int start = idx + 16;
...
void drawWebUiScreen(bool mode_ap) {
    ...
    if (mode_ap) {
        ...
        tft.drawCentreString("BruceModNet/brucemod", tftWidth / 2, 7, 1);
    }
    setTftDisplay(0, 0, ALCOLOR, FM);
    tft.drawCentreString("BruceMod WebUI", tftWidth / 2, 27, 1);
    String txt;
...
void serveWebUIFile(
    ...
    FS *fs = NULL;
    if (setupSdCard()) {
        if (SD.exists("/BruceModWebUI/" + filename)) fs = &SD;
    } else if (LittleFS.exists("/BruceModWebUI/" + filename)) {
        fs = &LittleFS;
    }
    if (fs) {
        response = request->beginResponse(*fs, "/BruceModWebUI/" + filename, contentType);
    } else {
...
void configureWebServer() {
    ...
    server->on("/login", HTTP_POST, [](AsyncWebServerRequest *request) {
        if (request->hasParam("username", true) && request->hasParam("password", true)) {
            ...
            if (username == bruceConfig.webUI.user && password == bruceConfig.webUI.pwd) {
                ...
                response->addHeader("Set-Cookie", "BRUCEMODSESSION=" + token + "; Path=/; HttpOnly");
                request->send(response);
    ...
    server->on("/logout", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        response->addHeader("Set-Cookie", "BRUCEMODSESSION=0; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT");
        request->send(response);
    });
