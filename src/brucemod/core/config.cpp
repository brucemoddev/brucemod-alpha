...
JsonDocument BruceConfig::toJson() const {
    ...
    setting["subMenuStyle"] = subMenuStyle;
    setting["debugMode"] = debugMode;
    setting["batteryWarningShown"] = batteryWarningShown;
    setting["statusLight"] = statusLight;
...
void BruceConfig::fromFile(bool checkFS) {
    ...
    if (!setting["subMenuStyle"].isNull()) {
        subMenuStyle = setting["subMenuStyle"].as<int>();
    } else {
        subMenuStyle = 5; // Default to C64 for submenus as per original Bruce behavior
    }

    if (!setting["debugMode"].isNull()) {
        debugMode = setting["debugMode"].as<bool>();
    } else {
        debugMode = false;
    }

    if (!setting["batteryWarningShown"].isNull()) {
        batteryWarningShown = setting["batteryWarningShown"].as<bool>();
    } else {
        batteryWarningShown = false;
    }

    if (!setting["statusLight"].isNull()) {
        statusLight = setting["statusLight"].as<bool>();
    } else {
        statusLight = false;
    }
...
void BruceConfig::validateConfig() {
    ...
    validateMenuStyleValue();
    validateSubMenuStyleValue();
...
void BruceConfig::setMenuStyle(int value) {
    menuStyle = value;
    validateMenuStyleValue();
    saveFile();
}

void BruceConfig::validateMenuStyleValue() {
    if (menuStyle < 0 || menuStyle > 5) menuStyle = 0;
}

void BruceConfig::setSubMenuStyle(int value) {
    subMenuStyle = value;
    validateSubMenuStyleValue();
    saveFile();
}

void BruceConfig::validateSubMenuStyleValue() {
    if (subMenuStyle < 0 || subMenuStyle > 5) subMenuStyle = 5;
}

void BruceConfig::setDebugMode(bool value) {
    debugMode = value;
    saveFile();
}

void BruceConfig::setBatteryWarningShown(bool value) {
    batteryWarningShown = value;
    saveFile();
}

void BruceConfig::setStatusLight(bool value) {
    statusLight = value;
    saveFile();
}
