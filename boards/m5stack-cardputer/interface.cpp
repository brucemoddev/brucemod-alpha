...
void InputHandler(void) {
    ...
    if (digitalRead(0) == LOW) { // GPIO0 button, shoulder button
        if (g0_press_start == 0) g0_press_start = millis();
        
        if (millis() - g0_press_start > 4000) {
            returnToMenu = true;
            AnyKeyPress = true;
            EscPress = true;
            displayWarning("FORCE CLOSE", true);
            g0_press_start = millis(); // reset to avoid multiple triggers
        }

        tm = millis();
        // Removed: SelPress = true mapping for G0
        AnyKeyPress = true;
    } else {
        g0_press_start = 0;
    }
